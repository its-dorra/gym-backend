...
export const attendanceTable = sqliteTable("attendance", {
  ...
  checkInTime: int("check_in_time", { mode: "timestamp" }).notNull().$default(() => new Date()),
...
export const insertAttendanceSchema = createInsertSchema(attendanceTable).omit({ id: true, deletedAt: true, createdAt: true, updatedAt: true });
