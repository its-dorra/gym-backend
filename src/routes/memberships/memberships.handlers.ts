...
import { createMembership, deleteMembership, getOneMembership, updateMembership } from "@/data-access/membership";
import { catchErrorTyped } from "@/lib/utils";

import type { CreateRoute, GetOneRoute, PatchRoute, RemoveRoute } from "./memberships.routes";


// export const memberHistory = A

export const create: AppRouteHandler<CreateRoute> = async (c) => {
  const membership = c.req.valid("json");
  const { id: userId } = c.var.user;

  const [insertedMembership, createMembershipError] = await catchErrorTyped(createMembership(userId, membership));

  if (createMembershipError)
    return c.json({ message: "Error in creating " }, HttpStatusCodes.INTERNAL_SERVER_ERROR);
...
export const getOne: AppRouteHandler<GetOneRoute> = async (c) => {
  ...
  if (getMembershipError)
    return c.json({ message: "Error getting the requested membership" }, HttpStatusCodes.INTERNAL_SERVER_ERROR);
...
export const patch: AppRouteHandler<PatchRoute> = async (c) => {
  ...
  if (updateMembershipError)
    return c.json({ message: "Error updating the membership" }, HttpStatusCodes.INTERNAL_SERVER_ERROR);
...
export const remove: AppRouteHandler<RemoveRoute> = async (c) => {
  ...
  if (removeMembershipError)
    return c.json({ message: "Error removing membership" }, HttpStatusCodes.INTERNAL_SERVER_ERROR);
